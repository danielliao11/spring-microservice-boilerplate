package com.saintdan.framework.starter.filter;

import com.saintdan.framework.common.constant.CommonsConstant;
import com.saintdan.framework.common.enums.CacheType;
import com.saintdan.framework.common.servlet.RequestWrapper;
import com.saintdan.framework.common.tools.LogUtils;
import com.saintdan.framework.common.tools.RemoteAddressUtils;
import com.saintdan.framework.starter.param.RequestCount;
...
public class LimitFilter implements Filter {
  ...
  private boolean limitWithRedis(RequestLimit requestLimit) {
    ...
    if (!limitRedisTemplate.hasKey(key)) {
      ...
      long frequency = System.currentTimeMillis() - (requestCount != null ? requestCount.getFirstReqAt() : 0);
      if ((requestCount != null ? requestCount.getCount() : 0) >= requestLimit.count && frequency <= requestLimit.range) {
        return false;
      } else {
